...
const quiz = {
    ...
    loadQuestion() {
        ...
        if (quiz.type[quiz.language][quiz.step] == 0) {
            ...
            quiz.answer[quiz.language][quiz.step].forEach((answerText) => {
                const answerElement = quiz.createAnswerElement(answerText);
            ...
            inputElement.addEventListener("change", (input) => {
                if (input.target.value == quiz.rightChoices[quiz.language][quiz.step]) {
                    quiz.resetColors();
    ...
    triggerHover(element, trigger, operationType) {
    ...
    pushToArray(index, object) {
        quiz.questions[index].push(object.question);
        quiz.answer[index].push(object.answers);
        quiz.rightChoices[index].push(object.correctAnswer);
        quiz.type[index].push(object.type);
    },
    ...
    createAnswerElement(answerText) {
        ...
        if (!quiz.isClicked) {
            ...
            quiz.triggerHover(answerElement, "mouseover", "add");
            quiz.triggerHover(answerElement, "mouseleave", "remove");
...
fetch('quiz.json')
    ...
    .then(data => {
        data.questions.forEach((object) => {
            quiz.pushToArray(object.language, object);
        });
    })
